...
SELECT_DDR      = $FB           ; AND to clear bit 2 -> access DDR
SELECT_PR       = $04           ; OR  to set   bit 2 -> access Peripheral Register
...
; ---- ClearMouse / HomeMouse / MouseAckIRQ / MouseSetConfig --------------
ClearMouse:
    lda #CMD_CLEAR
    bne SEND_B3                 ; always
HomeMouse:
...
; ---- ServeMouse: slot derived by COMMON; T_SERVE sends CMD_SERVE -
...
; ---- ReadMouse / InitMouse / MouseCredits / MouseRWMemory ---------
ReadMouse:
    ldx #<(T_B6-1)
    bne COMMON                  ; always
InitMouse:
...
; ---- PosMouse / ClampMouse / MouseSetVBLData / MouseSetVBLFrames -------------
...
; reads latch the PAL sync bit (and disable the $C800 expansion ROM)
...
B1_FN0:                         ; print the credits
    ...
    ldy #$00
B1_13:
    lda Credits,y               ; read character from string
    beq B1_1D                   ; NUL: done
...
; ==================================================================
; B3: SetMouse, ServeMouse, ClearMouse, HomeMouse, MouseSetConfig, MouseAckIRQ
...
;   B6_FN2: ReadMouse -- send CMD_READ, read 5 bytes into the screen holes
...
; ==================================================================
; B7: multi-byte MCU write (ClampMouse, PosMouse, MouseSetVBLFrames, MouseSetVBLData)
