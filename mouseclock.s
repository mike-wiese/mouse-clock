...
;      AppleMouse and keep their published addresses. ROM_BANK ($067x) is no
...
;      are moved to the high end of the INBUF page ($02Cx) so they
;      cannot clobber mouse state -- DOUT in particular had to move to free
;      $0678 for MC_MODE. These 4 keep their screen-hole addresses for
;      compatibility:
;        * IRQEN1 ($047x): documented (rarely used)
;        * RMODE  ($05Fx): named in the ThunderClock user manual
;        * WMODE  ($057x): set right before use; kept put for existing code
;        * IRQEN2 ($07Fx): documented (rarely used)
;
...
CLOCK_DOUT      = $02C0         ; was $0678-$C0
CLOCK_LCNT1     = $02C1         ; was $06F8-$C0
CLOCK_LCNT2     = $02C2         ; was $0778-$C0
CLOCK_TENS      = $02C3         ; was $07F8-$C0
...
MC_IN:  pha                     ; mouse/clock KSW hook
        lda #<(T_MC_IN-1)
        bne SAVE_STATE          ; always
...
PRODOS_WRITE:                   ; ProDOS passes A = '#', but A is trashed on entry
        lda #$A3                ; restore A = '#'
MC_OUT: pha                     ; mouse/clock CSW hook
        lda #<(T_MC_OUT-1)
        bne SAVE_STATE          ; always
...
B1_13:  lda CreditsEnd-256,y    ; read character (negative-indexed off the end)
...
; ==================================================================
; B3: SetMouse, ServeMouse, ClearMouse, HomeMouse, StartTimer
; ==================================================================
...
B4_FN2: pla                     ; discard the char SAVE_STATE pushed
        lda MC_MODE,x
...
SETTIMDIG:
        ...
        beq LC8F4
        lda #10
LC8F4:  sta CLOCK_TENS          ; CLOCK_TENS = 0 if data-out was 0, else 10
        clc
        bcc FINALIZE            ; always
COMMITTIME:
...
LCA2C:  pla                     ; hours ones, then minutes ones, seconds ones
        jsr EMIT_DIGIT
        dey
        beq CHKAMPM
        lda #$BA                ; ':'
...
SHLOOP: lda RTC_CONTROL,y       ; get DATA OUT bit in bit 7
...
LOOP:   lda LATEST_YEAR_TABLE,y
        ...
        dey
        bpl LOOP
SKIP:   jsr SLOTXY              ; restore X = $Cn, Y = $n0
