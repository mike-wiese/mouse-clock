...
; Bank select values in bits 1-3
BANK0           = %00000000
BANK1           = %00000010
BANK2           = %00000100
BANK3           = %00000110
...
MOUSE_OUT:                      ; our CSW entry point
        ...
        .byte <HomeMouse
        .byte <InitMouse
        .byte <DiagMouse        ; + a.k.a. GetClamp, Mouse TN #7
        .byte <Copyright        ; * not documented
        .byte <TimeData         ; + Mouse TN #2
        .byte <SetVBLCnts       ; * not documented
        .byte <OptMouse         ; * not documented
        .byte <StartTimer       ; * not documented
...
; ==================================================================
; OptMouse *
; Set the MCU's MOUSE_OPTIONS byte (MCU memory location $5D):
...
B1_FN0: tya                     ; save Y
        ...
        ldy #$00
B1_13:  lda (ZP_TEMP1),y        ; read character from string
        beq B1_1D               ; NUL terminator: done
        jsr COUT                ; print character to screen
B1_1A:  iny                     ; advance to next character
        bne B1_13               ; loop
B1_1D:  pla                     ; pop saved zp bytes
        sta ZP_TEMP2
        pla
        sta ZP_TEMP1
        pla
...
; Reached after B1_A0 call #1 (sent the DiagMouse MOUSE_CMD byte).
; Now send the MCU address low byte (held in CLAMP_MIN_LO) via call #2.
; ==================================================================
B1_AB:  lda #<B1_B4-1           ; lo byte for the RTS jump
        pha
...
; ==================================================================
B1_B4:  lda #<B1_BD-1           ; lo byte for the RTS jump
        pha
...
; ==================================================================
B1_BD:  lda #<B1_TO_B0-1        ; lo byte for the RTS jump
        pha
...
B2_47:  lda ZP_TEMP1            ; save zp temps on stack
        pha
        lda ZP_TEMP2
        pha
        tya                     ; save Y
        ...
        ldy #$00
        sty ZP_TEMP1
B2_57:  lda #$00                ; A = 0: fill byte
B2_59:  sta (ZP_TEMP1),y        ; zero one byte
        iny
        bne B2_59
        inc ZP_TEMP2
        lda ZP_TEMP2
...
B2_88:  rts
B2_89:  bne B2_39               ; always (entered with A=$E3 from B2_D7)
; ------------------------------------------------------------------
...
;
; - When the 6502 reads anywhere in $C800–$CFFF (this firmware uses $CFFF),
;   I/O STROBE* goes low, then PAL pin 19 = Q3; when Q3 pulses within that cycle ->
;   rising edge on pin 1 -> the D flip-flop captures /D0 into SYNC LATCH. On an
;   Apple II, $CFFF doesn't drive the bus, so D0 is bit 0 of the floating-bus
;   byte the video scanner just read. The 6502 then reads SYNC LATCH at PIA Port
;   B bit 0.
;
...
;   consecutive hits confirm the video scanner is on the last line and a
;   VBL is about to start.
;
; The sync loop takes 44 cycles when the first check misses and the branch to
; the top of the loop is taken. A 44-cycle loop lands on video positions
; of only ONE parity, i.e. only even addresses or only odd addresses. Both
...
;
;  1. The 61-cycle (odd) detour. When the first check DOES land on the second
;     sentinel, the second check (16 cycles later) misses and the loop returns
;     the long way, through the second $CFFF read: 61 cycles, not 44. This
;     ODD cycle count flips the loop's phase parity.
;
;  2. The second sentinel byte $3FE0 is scanned by the video scanner TWICE, on
;     consecutive clock cycles, so it will be detected by both even and odd parity
;     loops. Sather UTAII p.5-11: "The first address of HBL is always addressed twice
;     consecutively, because H0-H5 is in the all zero state for two consecutive scans."
;     Per UTAII p.5-18, the first address of HBL for line 261 (in vertical blanking)
;     is $3FE0, our second sentinel!
;
; So an odd-phase loop won't hit the sentinels on the last visible line, but it
; WILL hit the odd-phase copy of the second sentinel via the line 261 horizontal
...
B2_8B:  lda #$01
        ...
        sta ZP_TEMP2
        sta ZP_TEMP3
B2_A6:                          ; This loop takes 44 cycles when the first sync latch check misses:
                                ; 27 from B2_A6..B2_C0, plus 17 from B2_C0..bcs B2_A6.
                                ; When the first check hits but the second misses, the longer
                                ; return through the second $CFFF is 61 cycles (44+17, odd);
                                ; that odd detour is what flips phase parity, see header.
...
B2_B8:  php                     ; [3]   waste 7 cycles, to match mid byte increment path
        plp                     ; [4]
B2_BA:  php                     ; [3]   waste 12 cycles, to match hi byte increment path
        plp                     ; [4]
        lda #$00                ; [2]
        lda $00                 ; [3]
B2_C0:  lda $CFFF               ; [4]   access $CFFF, which triggers the PAL to update sync latch
...
; ==================================================================
B2_E4:  lda TEXT                ; switch display back to text mode
        lda LORES
        clc
...
; ==================================================================
; BANK 3: SetMouse, ServeMouse, ClearMouse, HomeMouse, OptMouse, StartTimer
; Send command byte to the MCU; ServeMouse also reads a status byte back
; ==================================================================
        .org $0300

B3_FN0: lda MOUSE_CMD,x         ; check command: ServeMouse needs a response read after write
        cmp #CMD_SERVEMOUSE     ; is it ServeMouse ($20)?
...
B4_26:  pha                     ; push command byte to send to MCU
        lda #BANK6
        sta ROM_BANK,x
        lda #$00                ; function code 0
        pha
...
B4_54:  lda MOUSE_MODE,x
        ...
        pha
        lda #CMD_READMOUSE      ; MCU command: read mouse position + status
        pha
        lda #BANK6
        bne B4_9A               ; always
        .res  6, $FF
...
; ==================================================================
B5_44:  lda #$8D                ; A = CR character ($8D)
...
; ==================================================================
        ...
        beq B5_FN0
B5_FN1: cpx #$80                ; hi byte negative?
...
B5_91:  sta BCD_LO              ; BCD_LO = lo byte of abs(value)
        stx BCD_HI              ; BCD_HI = hi byte of abs(value)
        lda #PLUS_SIGN
        bcc B5_9D               ; C=0 (positive): keep '+'
        lda #MINUS_SIGN         ; C=1 (negative): use  '-'
B5_9D:  pha                     ; push sign character (+ or -) onto stack; stored at end after digits
...
B5_A3:  ldx #$11                ; 17-bit conversion loop: enough for 5 decimal digits of a 16-bit number
        lda #$00                ; start accumulator at 0
        clc
B5_A8:  rol                     ; roll binary fraction bit into BCD accumulator
        cmp #$0A                ; if >= 10, subtract and carry into next digit
        bcc B5_AF               ; less than 10: no borrow needed this step
        sbc #$0A
B5_AF:  rol BCD_LO              ; shift left; carry out of BCD_HI feeds next rol A
        rol BCD_HI
...
B5_C8:  pla                     ; pop sign or separator character (pushed in B5_9D / B5_A3)
...
B6_48:  bvs B6_51
        ...
        lsr
        lsr
B6_51:  clv                     ; clear V so bvc below is always taken
        sta ROM_BANK,x
        beq B6_59
        lda #$80                ; negative function code -> do RTS jump after bank switch
B6_59:  pha                     ; push function code ($80 or $00)
...
TO_B6_48:
        beq B6_48               ; always
B6_FN1: clv                     ; clear V: read 1 byte
CFG_PA_FOR_READ:
        lda PIA_CRA,y
        and #SELECT_DDR
        sta PIA_CRA,y           ; access DDRA
...
; ==================================================================
; BANK 7: ClampMouse, PosMouse, SetVBLCnts, TimeData
;
...
B7_FN1: lda MOUSE_CMD,x         ; what command are we sending to the MCU?
        cmp #CMD_POSMOUSE       ; is it PosMouse ($40)?
        beq B7_29               ; yes: send mouse X/Y coords
        cmp #CMD_CLAMPMOUSE     ; is it ClampMouse X ($60)?
        beq B7_18               ; yes: send clamp bytes
        cmp #CMD_CLAMPMOUSE+1   ; is it ClampMouse Y ($61)?
        beq B7_18               ; yes: send clamp bytes
        cmp #CMD_SETVBLCNTS     ; is it SetVBLCnts ($A0)?
...
B7_38:  pha                     ; push last data byte (CLAMP_MIN_LO or MOUSE_XLO)
        ...
        bne B7_5D               ; always
B7_41:  and #$0C                ; isolate command bits 3:2 (how many bytes to send)
        lsr
        lsr
        lsr                     ; bit 2 -> carry
        ...
        lda #$02                ; send 2 bytes
        bne B7_5D               ; always
B7_57:  lda MOUSE_CMD,x         ; push command byte as only data byte
        pha
        lda #$01                ; send 1 byte: command byte only
B7_5D:  sta MOUSE_CMD,x         ; save byte count -- WRITE_LOOP decrements this to 0
...
B7_86:  lsr                     ; shift carry to distinguish sub-cases
