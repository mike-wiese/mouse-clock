...
LC80C:  clv                     ; clear V -- marks this as a warm (non-init) call
...
CLK_DISPATCH:
        ...
        jmp PATCH_P8_YT         ; C=1 (CLOCK_READ path) -> patch ProDOS driver -> READ_TIME
                                ; [patched: was JMP READ_TIME]
...
LC8F4:  sta CLOCK_IRQEN2,x      ; CLOCK_IRQEN2 = 0 if data-out was 0, else 10
        clc
        bcc FINALIZE            ; always
...
LC906:  jsr CLK_SHIFT           ; read out remaining 9 nibbles
        pha
        dec CLOCK_LCNT1,x
        bne LC906
        lda #$0A
...
LC9BA:  lda #$A0                ; Apple II high-bit space
        sta $0205               ; back-patch buf[5]: "MM/DD;HH..." -> "MM/DD HH..."

LC9BF:  lda MTN_SUFFIX,y        ; emit ".000"
        beq FMTDONE
        jsr EMIT_CHAR
        iny
        bne LC9BF
...
EMITLO: jsr EMIT_DIGIT          ; emit low nibble of current field

LCA2C:  pla                     ; hours ones (first pass), then minutes ones, seconds ones
        jsr EMIT_DIGIT
        dey
        beq CHKAMPM
        lda #$BA                ; ':' | $80
...
LCA7B:  ldx #$0C                ; re-emit at buffer offset $0C
        pha
        jsr ROR4
        and #$0F
        jsr EMIT_NOZERO
...
FMTFINAL:
;   Terminates the input with a CR, and reconstructs
;   the register stack so that the CR is returned in A
;   to the original caller.
        ...
        dex                     ; fix length
        pha                     ; RESTORE_STATE pops this as A (char returned to caller)
        tya
        pha                     ; RESTORE_STATE pops this as Y
        txa
        pha                     ; RESTORE_STATE pops this as X
        jmp FINALIZE
...
; $CC00-$CFFF  UPPER ROM
;
;   $CC00-$CC06  LATEST_YEAR_TABLE -- replacement year table data
;   $CC07-$CC26  PATCH_P8_YT -- year table patch routine
;   $CC27-$CFFF  Unprogrammed ($FF fill)
; ============================================================================

; ============================================================================
; $CC00  LATEST_YEAR_TABLE -- Updated ProDOS clock driver year table
;
;   Replacement for YEAR_TABLE in the ProDOS driver at $D7B8.
; ============================================================================
...
LATEST_YEAR_TABLE:
        ...
        nop                     ; help monitor listings get in sync
        nop                     ; help monitor listings get in sync

; ============================================================================
; $CC07  PATCH_P8_YT -- Update ProDOS clock driver year table
;
;   Entered via JMP from $C84D (patched from JMP READ_TIME).
;   Called on every CLOCK_READ.
;
;   Checks whether the caller is the ProDOS clock driver by examining the
;   return address on the stack. Address $BF06 in the ProDOS Global Page is
;   documented to contain JMP <clock driver>, so the driver's address (lo/hi)
;   is in $BF07/$BF08. The JSR CLOCK_READ is 12 bytes into the driver, so the
;   return address is base+14.
;
;   At entry the stack frame is:
;     SP+7  hi byte of JSR CLOCK_READ return address
;     SP+6  lo byte of JSR CLOCK_READ return address
;     ...
;
; ============================================================================
...
PATCH_P8_YT:
        ...
        cmp $0107,x             ; matches actual hi byte on stack?
        bne SKIP                ; no -> skip update
        tya                     ; restore lo result
        cmp $0106,x             ; matches actual lo byte on stack?
        bne SKIP                ; no -> skip update
; Store a pointer to the driver's YEAR_TABLE in $3A/$3B.
; $3A/$3B are not preserved since the driver overwrites them anyway.
        lda $BF07               ; lo byte of driver address
        clc
...
LOOP:   lda LATEST_YEAR_TABLE,y
        sta ($3A),y             ; -> driver's YEAR_TABLE[Y]
        dey
        bpl LOOP
SKIP:   ldx MSLOT               ; restore X = $Cn
        ldy SLOT16              ; restore Y = slot# * $10
; READ_TIME begins with PLA, no need to restore A here.
        jmp READ_TIME           ; -> read clock, format output string

        .res $03BF, $FF         ; $CC41-$CFFF unprogrammed
